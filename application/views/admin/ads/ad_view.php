...
<script type="text/javascript">
						...
						$('.delete').click(function(){
                            del=confirm('هل تريد حذف هذا الاعلان؟');
                            if(del)
                            {
                                var $t=$(this).attr('rel');
                                var $v='<td colspan="4" style="text-align:center;"><img src="<?php echo base_url(); ?>images/admin/loadingAnimation.gif"/></td>';
                                $('tr#'+$t).html($v);
                                $.post('<?php echo base_url(); ?>webadmin/ads/delete/'+$t,function(data){
                                    $('tr#'+$t).remove();
                                    if($('#myTable > tbody > tr').length==0)
                                    {
                                        $('#myTable > tbody').html('<tr><td colspan="5" alig="center">لا يوجد اعلانات حاليا</td></tr>');
                                    }
                                });
                            }
                        });
						$('#acts').click(function(){
                            ...
                            if(action!="-1")
                            {
                                if($('[name="chk[]"]:checked').length=='0')
                                {
                                    ...
                                    del=confirm('هل تريد اجراء هذة العملية علي الاعلانات  المختارة');
                                    if(del)
                                    {
                                        $('#forn').submit();
                                        $(this).val('-1');
	...
	<div class="container">
		...
		<div class="button ">
			<b>
				<a href="<?php echo base_url() ;?>webadmin/ads/add">
					<button class="button-item">اعلان جديد<img src="<?php echo base_url() ;?>images/admin/plus-small.gif" border="0" /></button>
				</a>
			</b>
		</div>
		...
		<div class="col-md-8 col-12 mr-md-2 wrapper" >
			<p class="top"  style="text-align:right;">الاعلانات</p>
			<div class="module-body">																		
				<div class="module-table-body w-100">
					<form action="<?php echo base_url(); ?>webadmin/ads/do_operation" id="forn" method="post">
						<table width="99%" class="tablesorter" id="myTable" style="background-position:center;">
							<thead>
								<tr>
									<th style="width:5%">#</th>
									<th class="w-50">عنوان الاعلان</th>
							 ...
							 <?php
								if(!empty($ads)){
									for ($i = 0; $i < count($ads); $i++) {
										...
										echo '<td><a href="' . base_url() . 'webadmin/ads/edit/' . $ads[$i]->ad_id . '">' . $ads[$i]->ad_name . '</a></td>';
										
										$ad_start=$ads[$i]->ad_start;
										$ad_start_time =strtotime($ad_start);
										$date= time();
										if($ads[$i]->statue==1){
											echo '<td><a href="'. base_url().'webadmin/ads/edit/'.$ads[$i]->ad_id.'"><b>معروض</b></td>';
										}elseif($ad_start_time>=$date){
											echo '<td><a href="'. base_url().'webadmin/ads/edit/'.$ads[$i]->ad_id.'">لم يعرض بعد</td>';
										}else{
											echo '<td><a href="'. base_url().'webadmin/ads/edit/'.$ads[$i]->ad_id.'">انتهي</td>';
										}
										echo '<td>';
										echo '<input type="checkbox" name="chk[]" value="'.$ads[$i]->ad_id.'" />';
										echo '<a href="' . base_url() . 'webadmin/ads/edit/' . $ads[$i]->ad_id . '"><img src="' . base_url() . 'images/admin/pencil.gif"  width="16" height="16" alt="edit" /></a>';
										...
										echo '<td><strong>لا يوجد اعلانات حاليا</strong></td>';
										echo '<td>';
										echo '</td>';
										echo '</tr>';							
									}			
