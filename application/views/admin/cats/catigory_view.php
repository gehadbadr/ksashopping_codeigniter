...
<script type="text/javascript">
						...
						$('.delete').click(function(){
                            del=confirm('هل تريد حذف هذا التصنيف ؟');
                            if(del)
                            {
                                var $t=$(this).attr('rel');
                                ...
                                $.post('<?php echo base_url(); ?>webadmin/catigory/delete/'+$t,function(data){
                                    ...
                                    {
                                        $('#myTable > tbody').html('<tr><td colspan="5" alig="center">لا يوجد تصنيفات حاليا</td></tr>');
                                    }
                                });
                            }
                        });
						$('#acts').click(function(){
                            ...
                            if(action!="-1")
                            {
                                if($('[name="chk[]"]:checked').length=='0')
                                {
                                    ...
                                    del=confirm('هل تريد اجراء هذة العملية علي التصنيفات المختارة');
                                    if(del)
                                    {
                                        $('#forn').submit();
                                        $(this).val('-1');
	...
	<div class="container">
		...
		<div class="button ">
			...
			<b><a href="<?php echo base_url() ;?>webadmin/catigory/add">
					<button class="button-item">تصنيف جديد<img src="<?php echo base_url() ;?>images/admin/plus-small.gif" border="0" /></button>
				</a>
			</b>
		</div>
		...
		<div class="col-md-8 col-12 mr-md-2 wrapper" >
			<p class="top"  style="text-align:right;">التصنيفات</p>
			<div class="module-body">	
			<?php
			...
			<?php }?>				
				...
				<?php }elseif (!empty($this->session->flashdata('login_error'))){?>
						<input type="hidden" id="cat_id" name="cat_id" value="<?php echo $this->session->flashdata('cat_id'); ?>" />
				<?php }else{?>
				...
				<form action="<?php echo base_url(); ?>webadmin/catigory/do_sort" id="do_sort_form" method="post">
					</form>
							 ...
							 <?php
								 if(!empty($cats)){
									for ($i = 0; $i < count($cats); $i++) {
										...
										echo '<a class="up" href="#" rel="' . $cats[$i]->cat_id  . '"><img border="0" src="' . base_url() . '/images/admin/navigate_up.png" width="20" height="20"></a>';
										echo '<a class="down" href="#" rel="' . $cats[$i]->cat_id  . '"><img border="0" src="' . base_url() . '/images/admin/navigate_down.png" width="20" height="20" ></a>';
										echo '<a class="navbar-toggler write p-0" href="#" rel="' . $cats[$i]->cat_id . '" data-toggle="collapse" data-target="#slidingDiv"><img border="0" src="' . base_url() . '/images/admin/k-write.png" width="20" height="20"></a>';
										...
										echo '<td><strong>لا يوجد تصنيف حاليا</strong></td>';
										echo '<td>';
										echo '</td>';
										echo '</tr>';							
								}			
	...
	<?php  $this->load->view('admin/includes/footer.php'); ?>       
				...
				<script type="text/javascript">
					...
					$('.write').click(function(){
						...
						var $t=$(this).attr('rel');
						document.getElementById("cat_id").value = $t;
