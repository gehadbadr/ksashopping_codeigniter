...
<script type="text/javascript">
						...
						$('.delete').click(function(){
                            del=confirm('هل تريد حذف هذة الشريحة؟');
                            if(del)
                            {
                                var $t=$(this).attr('rel');
                                ...
                                $.post('<?php echo base_url(); ?>webadmin/slider/delete/'+$t,function(data){
                                    ...
                                    {
                                        $('#myTable > tbody').html('<tr><td colspan="5" alig="center">لا يوجد شرائح حاليا</td></tr>');
                                    }
                                });
                            }
                        });
						$('#acts').click(function(){
                            ...
                            if(action!="-1")
                            {
                                if($('[name="chk[]"]:checked').length=='0')
                                {
                                    ...
                                    del=confirm('هل تريد اجراء هذة العملية علي الشرائح  المختارة');
                                    if(del)
                                    {
                                        $('#forn').submit();
                                        $(this).val('-1');
	...
	<div class="container">
		...
		<div class="button ">
			<b>
				<a href="<?php echo base_url() ;?>webadmin/slider/add">
					<button class="button-item">شريحة جديدة<img src="<?php echo base_url() ;?>images/admin/plus-small.gif" border="0" /></button>
				</a>
			</b>
		</div>
		...
		<div class="col-md-8 col-12 mr-md-2 wrapper" >
			<p class="top"  style="text-align:right;">الشرائح</p>
			<div class="module-body">																		
					...
					<form action="<?php echo base_url(); ?>webadmin/slider/do_operation" id="forn" method="post">
						<table width="99%" class="tablesorter" id="myTable" style="background-position:center;">
							<thead>
								<tr>
									<th style="width:5%">#</th>
									<th class="w-50">عنوان الشريحة</th>
